...
description: Reflex.dev state design, event handlers, background tasks, form validation, page factory, service registry, repository pattern, database models, project architecture.
metadata:
...
Every feature lives in its own workspace package under `components/<project>-<name>/`. The main `app/` is the shell that assembles packages into a running application.

```
components/<project>-<feature>/
└── src/<project>_<feature>/
    ├── __init__.py          # public exports only
    ├── configuration.py     # Pydantic config schemas
    ├── backend/             # or backend.py — pure Python, no Reflex
    │   ├── entities.py      # DB models
    │   ├── models.py        # UI models
    │   ├── repositories.py  # async DB access
    │   └── services/        # business logic
    ├── state/               # or state.py — rx.State subclasses
    ├── components/          # or components.py — UI components
...
**Rule**: business logic belongs in `backend/`, reactive state in `state/`, UI in `components/`, routing in `pages.py`.
Never mix concerns.
